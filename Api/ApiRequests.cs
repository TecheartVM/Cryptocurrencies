...
namespace CryptocurrenciesWPF.Api
{
    ...
    {
        public static async Task<CoinModel[]> GetTopCoinsAsync(uint count, string priceIn = "usd")
        {
        ...
        public static async Task<CoinInfoModel> GetCoinAsync(string coinId)
        {
            return await ApiHelper.GetObjectFromApiAsync<CoinInfoModel>(
                $"coins/{coinId}?tickers=false&community_data=false&developer_data=false");
        }

        public static async Task<CoinInfoModel> GetCoinAsync(string coinId, string localization)
        {
            return await ApiHelper.GetObjectFromApiAsync<CoinInfoModel>(
                $"coins/{coinId}?localization={localization}&tickers=false&community_data=false&developer_data=false");
        }
